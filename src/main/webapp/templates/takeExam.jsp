...
<div class="progress">
    ...
    <s:form action="submitAnswer">
      ...
      <s:iterator value="question.answers" status="it">
        <div class="answer input-group">
          <span class="input-group-addon"><input required type="radio" name="sc" value="${it.index}" ${examResult.findQuestionById(questionId) != null && examResult.findSubmittedAnswersByQuestionId(questionId).getAnswers().toArray()[it.index].rightAnswer ? 'checked' : ''}></span>
  ...
  <s:elseif test="%{question.type == @de.nordakademie.iaa_multiple_choice.domain.QuestionType@MULTIPLE_CHOICE}">
    ...
    <s:form action="submitAnswer">
      ...
      <s:iterator value="question.answers" status="it">
        <div class="answer input-group">
          <span class="input-group-addon"><input type="checkbox" name="mc" value="${it.index}" ${examResult.findQuestionById(questionId) != null && examResult.findSubmittedAnswersByQuestionId(questionId).getAnswers().toArray()[it.index].rightAnswer ? 'checked' : ''}></span>
          <span class="form-control"><s:property value="text"/></span>
        </div>
      </s:iterator>
      <div class="center">
    ...
    <s:form action="submitAnswer">
      ...
      <s:iterator value='question.getFormattedQuestionText().split("\\\[\\\]")' status="it">
        ...
        <s:if test="#it.last == false || question.getFormattedQuestionText().endsWith('[]')">
          <s:textfield class="form-group blank" name="fillInTheBlankAnswers" type="text" required="true" value="%{examResult.findQuestionById(questionId) == null ? '' : examResult.findSubmittedAnswersByQuestionId(questionId).getAnswers().toArray()[#it.index].getText()}" />
        </s:if>
      </s:iterator>
      <div class="center">
        <s:submit class="btn btn-success" value="%{getText('submit.answer')}" />
