<%-- authors: Yannick Rump, Hannes Peterson and Tim Brust --%> <%@ page language="java" contentType="text/html; charset=UTF-8"
pageEncoding="UTF-8"%> <%@ taglib uri="/struts-tags" prefix="s"%> <%@ taglib uri="http://java.sun.com/jsp/jstl/functions" prefix="fn" %>
...
<s:if test="exam != null"> </s:if>
<s:form>
	<s:if test="hasFieldErrors()">
		<div class="alert alert-danger" role="alert">
			<span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>
			<span class="sr-only"><s:text name="validation.error"/></span> <strong><s:text name="validation.errorIntro"/></strong>
			<s:fielderror />
		</div>
	</s:if>
	<s:hidden name="examId" value="%{#attr.examId}" />
	<div class="form-group ${fn:escapeXml(fieldErrors.containsKey('exam.name')) ? 'has-error' : ''}">
	...
	<div class="form-group ${fn:escapeXml(fieldErrors.containsKey('exam.examTime')) ? 'has-error' : ''}">
	...
	<div class="form-group ${fn:escapeXml(fieldErrors.containsKey('exam.minPoints')) ? 'has-error' : ''}">
	...
	<div class="form-group ${fn:escapeXml(fieldErrors.containsKey('exam.creditPoints')) ? 'has-error' : ''}">
		...
		<s:select
			...
			headerValue=""
			listValue="value"
			name="exam.creditPoints"
			list="@de.nordakademie.iaa_multiple_choice.domain.CreditPointType@values()"
		/>
	</div>
	<div class="form-group ${fn:escapeXml(fieldErrors.containsKey('exam.evaluationMethod')) ? 'has-error' : ''}">
		<label for="create.evaluationMethod"><s:text name="create.evaluationMethod"/></label>
		<s:select
			required="true"
			class="form-control"
			id="create.evaluationMethod"
			headerKey=""
			headerValue=""
			listValue="%{getText(text)}"
			name="exam.evaluationMethod"
			list="@de.nordakademie.iaa_multiple_choice.domain.WrongAnswerEvaluationMethod@values()"
		/>
	</div>
	<div class="form-group ${fn:escapeXml(fieldErrors.containsKey('exam.endDate')) ? 'has-error' : ''}">
		...
		<div class="input-daterange input-group" id="datepicker">
			<s:textfield type="text" class="input-sm form-control" name="rawStartDate" id="examStart" required="true" />
			<span class="input-group-addon"><s:text name="create.examPeriodTo"/></span>
			<s:textfield type="text" class="input-sm form-control" name="rawEndDate" id="examEnd" required="true" />
		</div>
	</div>
	<div class="center">
		<s:if test="examId == null">
...
<script type="text/javascript" src="/iaa-multiple-choice/static/js/bootstrap-datepicker.min.js"></script>
<script type="text/javascript" src="/iaa-multiple-choice/static/js/bootstrap-datepicker.de.min.js"></script>
